...
import cv2
import torch
from nn_isomorphics.utils import imgproc
...
class ImageDataset(Dataset):
    ...
    def __init__(self, image_dir: str, image_size: int, pre_transform) -> None:
    ...
    def __getitem__(self, batch_index: int) -> [torch.Tensor, int]:
        image_dir, image_name = self.image_file_paths[batch_index].split(
            self.delimiter)[-2:]
        ...
        if image_name.split(".")[-1].lower() in IMG_EXTENSIONS:
            ...
            raise ValueError(
                f"Unsupported image extensions, Only support `{IMG_EXTENSIONS}`, "
...
class PrefetchDataLoader(DataLoader):
    ...
    def __iter__(self):
        return PrefetchGenerator(super().__iter__(),
