...
import numpy as np
import torch
from nn_isomorphics.utils.utils import ClassificationPresetTrain
...
FI2010_DIR = 'data/fi2020/lob_raw.h5'
IMAGENET_DIR = "./datasets/imagenet/"

IMAGENET_TRANSFORM_CONFIG = {
    'train_crop_size': 224,
...
def get_imagenet_val(batch_size=32, num_workers=4) -> DataLoader:
    ...
    val_dataset = ImageNet(root=IMAGENET_DIR,
                           split='val',
...
def get_imagenet_train(batch_size=32, num_workers=4) -> DataLoader:

    train_transform = ClassificationPresetTrain(
        crop_size=IMAGENET_TRANSFORM_CONFIG['train_crop_size'],
        interpolation=IMAGENET_TRANSFORM_CONFIG['interpolation'],
        auto_augment_policy=IMAGENET_TRANSFORM_CONFIG['auto_augment_policy'],
        random_erase_prob=IMAGENET_TRANSFORM_CONFIG['random_erase_prob'],
        ra_magnitude=IMAGENET_TRANSFORM_CONFIG['ra_magnitude'],
        augmix_severity=IMAGENET_TRANSFORM_CONFIG['augmix_severity'],
    )

    train_dataset = ImageNet(root=IMAGENET_DIR,
                             split='train',
...
def get_mnist(batch_size,
    ...
    train_mnist = MNIST("./data/train",
                        transform=transforms,
                        train=True,
                        download=True)
    test_mnist = MNIST("./data/test",
                       transform=transforms,
                       train=False,
                       download=True)
...
def get_fi2010_loader(h5_path=FI2010_DIR,
                      window=50,
                      split=0,
                      horizon=0,
                      batch_size=128,
